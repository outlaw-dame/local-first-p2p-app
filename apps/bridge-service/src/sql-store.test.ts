...
describe('PgliteBridgeStore', () => {
  it('deduplicates idempotent deliveries without consuming extra sequence values', async () => {
    const service = new BridgeService({
      store: new PgliteBridgeStore({ initialSequence: 0, ttlMs: 60_000 })
    });
    const event = makeSignedEvent('evt_sql_duplicate');

    const [first, second] = await Promise.all([
      service.acceptDelivery(
        { idempotencyKey: 'idem-sql-duplicate', target: 'bridge:sql', event },
  ...
  it('does not advance sequence state when idempotency conflicts are rejected', async () => {
    ...
    await expect(
      service.acceptDelivery(
        { idempotencyKey: 'idem-sql-conflict', target: 'bridge:sql', event: first },
        '1970-01-01T00:00:00.000Z'
      )
    ...
    await expect(
      service.acceptDelivery(
        { idempotencyKey: 'idem-sql-conflict', target: 'bridge:sql', event: second },
  ...
  it('persists conflicts and evicts oldest records by capacity', async () => {
    const service = new BridgeService({
      store: new PgliteBridgeStore({ initialSequence: 0, maxRecords: 1, ttlMs: 60_000 })
    });
    ...
    await expect(
      service.acceptDelivery(
        { idempotencyKey: 'idem-sql-first', target: 'bridge:sql', event: first },
        '1970-01-01T00:00:00.000Z'
      )
    ...
    await expect(
      service.acceptDelivery(
        { idempotencyKey: 'idem-sql-second', target: 'bridge:sql', event: second },
        '1970-01-01T00:00:01.000Z'
      )
    ).resolves.toMatchObject({ status: 'confirmed' });

    await expect(
      service.getRecord('idem-sql-first', '1970-01-01T00:00:02.000Z')
    ).resolves.toBeUndefined();
    await expect(
      service.getRecord('idem-sql-second', '1970-01-01T00:00:02.000Z')
    ).resolves.toMatchObject({
  ...
  it('prunes expired records before reads and snapshots', async () => {
    const service = new BridgeService({
      store: new PgliteBridgeStore({ initialSequence: 0, ttlMs: 1_000 })
    });
    const event = makeSignedEvent('evt_sql_expiring');

    await service.acceptDelivery(
      { idempotencyKey: 'idem-sql-expiring', target: 'bridge:sql', event },
      '2026-05-22T00:00:00.000Z'
    );

    await expect(
      service.getRecord('idem-sql-expiring', '2026-05-22T00:00:02.000Z')
    ).resolves.toBeUndefined();
  ...
  it('persists readable signed events for inbound reads', async () => {
    ...
    await service.acceptDelivery(
      { idempotencyKey: 'idem-sql-read-1', target: 'bridge:sql', event: first },
      '1970-01-01T00:00:00.000Z'
    );
    await service.acceptDelivery(
      { idempotencyKey: 'idem-sql-read-2', target: 'bridge:sql', event: second },
      '1970-01-01T00:00:01.000Z'
    );

    await expect(
      service.readInboundRecords(
        {
