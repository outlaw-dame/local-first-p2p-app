...
describe('InMemoryBridgeService', () => {
  ...
  it('does not consume extra sequence values for concurrent duplicate deliveries', async () => {
    ...
    const [first, second] = await Promise.all([
      bridge.acceptDelivery(
        { idempotencyKey: 'idem-race', target: 'bridge:dev', event },
        '1970-01-01T00:00:00.000Z'
      ),
      bridge.acceptDelivery(
        { idempotencyKey: 'idem-race', target: 'bridge:dev', event },
    ...
    await expect(bridge.snapshot('1970-01-01T00:00:01.000Z')).resolves.toMatchObject({
      latestSequence: 1,
      acceptedCount: 1
    });
  });
  ...
  it('bounds in-memory idempotency records by capacity and TTL', async () => {
    ...
    await bridge.acceptDelivery(
      { idempotencyKey: 'idem-1', target: 'bridge:dev', event: first },
      '2026-05-22T00:00:00.000Z'
    );
    await bridge.acceptDelivery(
      { idempotencyKey: 'idem-2', target: 'bridge:dev', event: second },
      '2026-05-22T00:00:00.100Z'
    );
    await bridge.acceptDelivery(
      { idempotencyKey: 'idem-3', target: 'bridge:dev', event: third },
      '2026-05-22T00:00:00.200Z'
    );
  ...
  it('detects idempotency-key conflicts', async () => {
    ...
    await expect(
      bridge.acceptDelivery({ idempotencyKey: 'idem-conflict', target: 'bridge:dev', event: first })
...
describe('JsonFileBridgeStore', () => {
  it('persists idempotency records and sequence state across service instances', async () => {
    ...
    try {
      ...
      const next = await secondService.acceptDelivery(
        {
          ...
          event: makeSignedEvent({ eventId: 'evt_next', privacy: 'public' })
        },
      ...
      await expect(
        secondService.getRecord('idem-persisted', '1970-01-01T00:00:30.000Z')
      ).resolves.toMatchObject({ eventId: 'evt_persisted', sequence: 1 });
      await expect(secondService.snapshot('1970-01-01T00:00:40.000Z')).resolves.toMatchObject({
        storeKind: 'json-file',
        acceptedCount: 2,
  ...
  it('reloads durable state before writes so one instance cannot overwrite another instance', async () => {
    ...
    const filePath = join(dir, 'bridge-store.json');
    try {
      const serviceA = new BridgeService({
        store: new JsonFileBridgeStore({
          filePath,
          initialSequence: 0,
          maxRecords: 10,
      ...
      const serviceB = new BridgeService({
        store: new JsonFileBridgeStore({
          filePath,
          initialSequence: 0,
          maxRecords: 10,
      ...
      await serviceB.acceptDelivery(
        {
          ...
          target: 'bridge:durable',
          event: makeSignedEvent({ eventId: 'evt_b', privacy: 'public' })
        },
      ...
      await serviceA.acceptDelivery(
        {
          ...
          target: 'bridge:durable',
          event: makeSignedEvent({ eventId: 'evt_a', privacy: 'public' })
        },
  ...
  it('keeps snapshot read-only while explicit prune persists cleanup', async () => {
    ...
    const filePath = join(dir, 'bridge-store.json');
    try {
      const store = new JsonFileBridgeStore({
        filePath,
        initialSequence: 0,
        maxRecords: 10,
        ttlMs: 1_000
      ...
      const afterExplicitPrune = JSON.parse(await readFile(filePath, 'utf8')) as {
        records: unknown[];
      };
...
describe('handleBridgeDeliveryRequest', () => {
  ...
  it('rejects malformed or inconsistent HTTP delivery requests', async () => {
    ...
    const mismatched = await handleBridgeDeliveryRequest(
      ...
      makeRequest(
        'idem-body',
        makeSignedEvent({ eventId: 'evt_http_mismatch', privacy: 'public' }),
        'idem-header'
      ),
      '2026-05-22T00:00:00.000Z'
    );
    ...
    expect(await mismatched.json()).toMatchObject({
      status: 'rejected',
      reason: 'Idempotency header does not match request body'
    });
  ...
  it('enforces optional HTTP auth before parsing delivery bodies', async () => {
    ...
    const wrong = await handleBridgeDeliveryRequest(
      ...
      makeRequest(
        'idem-auth-wrong',
        makeSignedEvent({ eventId: 'evt_auth_wrong', privacy: 'public' }),
        undefined,
        'wrong-value'
      ),
      '2026-05-22T00:00:00.000Z',
  ...
  it('accepts authorized delivery requests without leaking server auth config', async () => {
    ...
    const authorized = await handleBridgeDeliveryRequest(
      ...
      makeRequest(
        'idem-auth-ok',
        makeSignedEvent({ eventId: 'evt_auth_ok', privacy: 'public' }),
        undefined,
        BRIDGE_AUTH_TOKEN
      ),
      '2026-05-22T00:00:00.000Z',
    ...
    const misconfigured = await handleBridgeDeliveryRequest(
      bridge,
      makeRequest(
        'idem-auth-bad-config',
        makeSignedEvent({ eventId: 'evt_auth_bad_config', privacy: 'public' }),
        undefined,
        BRIDGE_AUTH_TOKEN
      ),
      '2026-05-22T00:00:00.000Z',
  ...
  it('accepts authorization scheme casing allowed by HTTP auth semantics', async () => {
    ...
    const response = await handleBridgeDeliveryRequest(
      bridge,
      makeRequest(
        'idem-auth-lowercase-scheme',
        makeSignedEvent({ eventId: 'evt_auth_lowercase_scheme', privacy: 'public' }),
        undefined,
        BRIDGE_AUTH_TOKEN,
        'bearer'
      ),
    ...
    await expect(response.json()).resolves.toMatchObject({
      status: 'confirmed',
      idempotencyKey: 'idem-auth-lowercase-scheme'
    });
  });
  ...
  it('treats malformed runtime auth options as generic server misconfiguration', async () => {
    ...
    const nonStringToken = await handleBridgeDeliveryRequest(
      bridge,
      makeRequest(
        'idem-auth-non-string',
        makeSignedEvent({ eventId: 'evt_auth_non_string', privacy: 'public' }),
        undefined,
        BRIDGE_AUTH_TOKEN
      ),
      '2026-05-22T00:00:00.000Z',
      { auth: { scheme: 'bearer', token: 123 } } as unknown as Parameters<
        typeof handleBridgeDeliveryRequest
      >[3]
    );
    const nonAsciiToken = await handleBridgeDeliveryRequest(
      ...
      makeRequest(
        'idem-auth-non-ascii',
        makeSignedEvent({ eventId: 'evt_auth_non_ascii', privacy: 'public' }),
        undefined,
        BRIDGE_AUTH_TOKEN
      ),
      '2026-05-22T00:00:00.000Z',
      { auth: { scheme: 'bearer', token: nonAsciiAuthToken } } as unknown as Parameters<
        typeof handleBridgeDeliveryRequest
      >[3]
    );
    const nullOptions = await handleBridgeDeliveryRequest(
      bridge,
      makeRequest(
        'idem-auth-null-options',
        makeSignedEvent({ eventId: 'evt_auth_null_options', privacy: 'public' }),
        undefined,
        BRIDGE_AUTH_TOKEN
      ),
      '2026-05-22T00:00:00.000Z',
