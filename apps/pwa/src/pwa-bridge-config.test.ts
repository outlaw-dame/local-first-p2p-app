...
describe('PWA bridge config boundary', () => {
  ...
  it('requires an endpoint when enabled', () => {
    expect(resolvePwaBridgeConfig(ENABLED_ENV)).toMatchObject({ status: 'invalid', reason: 'missing-endpoint' });
  });
  ...
  it('allows local http endpoints for development only', () => {
    expect(
      resolvePwaBridgeConfig({ ...ENABLED_ENV, VITE_LFP2P_BRIDGE_ENDPOINT: 'http://localhost:8787/events' })
    ).toMatchObject({ status: 'configured', endpoint: 'http://localhost:8787/events' });
    expect(
      resolvePwaBridgeConfig({ ...ENABLED_ENV, VITE_LFP2P_BRIDGE_ENDPOINT: 'http://127.0.0.1:8787/events' })
    ).toMatchObject({ status: 'configured', endpoint: 'http://127.0.0.1:8787/events' });
  });

  it('rejects insecure remote endpoints', () => {
    expect(resolvePwaBridgeConfig({ ...ENABLED_ENV, VITE_LFP2P_BRIDGE_ENDPOINT: 'http://bridge.example.test/events' })).toMatchObject({
      status: 'invalid',
  ...
  it('rejects endpoints with credentials, query strings, fragments, or unsupported protocols', () => {
    expect(resolvePwaBridgeConfig({ ...ENABLED_ENV, VITE_LFP2P_BRIDGE_ENDPOINT: 'https://user:pass@bridge.test/events' })).toMatchObject({
      status: 'invalid',
      reason: 'embedded-credentials'
    });
    expect(resolvePwaBridgeConfig({ ...ENABLED_ENV, VITE_LFP2P_BRIDGE_ENDPOINT: 'https://bridge.test/events?token=secret' })).toMatchObject({
      status: 'invalid',
      reason: 'query-or-fragment'
    });
    expect(resolvePwaBridgeConfig({ ...ENABLED_ENV, VITE_LFP2P_BRIDGE_ENDPOINT: 'https://bridge.test/events#secret' })).toMatchObject({
      status: 'invalid',
      reason: 'query-or-fragment'
    });
    expect(resolvePwaBridgeConfig({ ...ENABLED_ENV, VITE_LFP2P_BRIDGE_ENDPOINT: 'wss://bridge.test/events' })).toMatchObject({
      status: 'invalid',
  ...
  it('validates timeout and target configuration', () => {
    expect(
      resolvePwaBridgeConfig({ ...ENABLED_ENV, VITE_LFP2P_BRIDGE_ENDPOINT: 'https://bridge.test/events', VITE_LFP2P_BRIDGE_TIMEOUT_MS: '0' })
    ).toMatchObject({ status: 'invalid', reason: 'invalid-timeout' });
    expect(
      resolvePwaBridgeConfig({ ...ENABLED_ENV, VITE_LFP2P_BRIDGE_ENDPOINT: 'https://bridge.test/events', VITE_LFP2P_BRIDGE_TIMEOUT_MS: '90000' })
    ).toMatchObject({ status: 'invalid', reason: 'invalid-timeout' });
    expect(
      resolvePwaBridgeConfig({ ...ENABLED_ENV, VITE_LFP2P_BRIDGE_ENDPOINT: 'https://bridge.test/events', VITE_LFP2P_BRIDGE_TARGET: 'bridge primary' })
