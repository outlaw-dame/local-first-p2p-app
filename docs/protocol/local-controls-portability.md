...
| A user-supplied regex in a keyword filter causes ReDoS | `matchKind` enum does not include `regex`; `semantic` requires a precomputed embedding ref so no regex is ever compiled against user text. The `phrase` and `hashtag` kinds (Phase 1.70.A) live on the linear-time path; only two constant patterns (`/\s+/g` and `/^[\p{L}\p{N}_]+$/u`) are compiled, and they are compiled once at module load against literal source — never against caller text. Adversarial timing tests pin these matchers at under 200 ms on a `#` + 20 000 `a`'s payload. |
