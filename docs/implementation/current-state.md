...
Phase 1.8.1 — reputation graph protocol layer (first slice of the Phase 1.8 doctrine):

- New sub-module `@lfp2p/trust-safety/reputation-graph` ships five canonical event payload types + `validateReputationEvent` pure validator. Five enums frozen at module load: `OBSERVATION_KINDS` (7 — outbox/bridge/media), `ATTESTATION_VALENCES` (3 — positive/negative/dispute), `ATTESTATION_CONTEXT_TAGS` (10 — contact/community/commercial, both positive and adverse mirrors), `AGGREGATOR_REMOVAL_REASONS` (4), `REPUTATION_ALGORITHMS` (3, including `local.personalized-eigentrust.v1` reserved for Phase 1.8.2's device-side computation). `REPUTATION_LIMITS`: `maxSubjectsPerAggregatorBatch: 10_000`, `maxWindowMs: 365 days`, `maxObservationCount: 1_000_000`.
- One new error code `TS_INVALID_REPUTATION` covers reputation-specific composite-shape failures (window > maxWindowMs, both-zero counts, empty aggregator batch). Other failure modes reuse existing `TS_INVALID_ENUM` / `TS_INVALID_NUMBER` / `TS_INVALID_TIMESTAMP` / `TS_INVALID_SUBJECT` etc. so the package error namespace stays small.
- Validator enforces (doctrine non-negotiable #5): every string field is `assertOneOf` against a frozen enum (no free-form text anywhere on a reputation event); every numeric field range-checked (NaN/Infinity rejected); ISO-8601 timestamps require explicit timezone designator; lifecycle invariants windowStart ≤ windowEnd, window duration ≤ maxWindowMs, expiresAt ≥ createdAt, revokedAt ≥ createdAt, **computedAt ≤ createdAt** (clock-skew sentinel — a future-dated aggregator computation is a forged-clock signal); both-zero counts rejected; empty aggregator batch rejected; subject ref delegated to `validateSafetySubjectRef`; output deep-frozen at every level (Phase 3.2); prototype-pollution defense at every payload-object boundary including nested aggregator subjects (matches Phase 1.71 + Phase 2.1).
- 20 valid + 10 invalid fixtures under `packages/trust-safety/fixtures/reputation-graph/` (4 valid + 2 invalid per kind exactly per doctrine).
- 79 new adversarial tests in `reputation-graph-events.test.ts`: fixture coverage (32), bounded-enum freeze + no-duplicate (2), forward-compat rejection across all 6 string-typed fields (6), numeric-range hardening with parameterized NaN/Inf/OOB (16), timestamp + lifecycle including clock-skew sentinel (7), prototype-pollution via JSON.parse delivery (5), subject-list cap (3), output-integrity walk (2), replay determinism + fixture round-trip (2), version pinning (2), subject-ref delegation (2), doctrine-#5 cross-check (1).

Phase 1.8.2 — local personalized EigenTrust computer:

- Pure `computeReputation(inputs) → LocalReputationState` in `packages/trust-safety/src/reputation-graph/computer.ts`. Follows the doctrine's personalized PageRank formulation `t̄^(k+1) = (1−α)·p + α·Cᵀ·t̄^(k)` with `p` seeded from the user's Phase 2.3 contact graph (closes the symmetric-reputation-function sybil result).
- Algorithm: apply revocations → drop observations outside window cutoff → aggregate raw (observer, subject) weights from observations (Kamvar `(sat − unsat) × timeDecay`) and attestations (`strength × valence-sign × timeDecay × 10`) → truncate to `maxNodes` deterministically by ascending stable-id sort → cap edges per observer at `maxEdgesPerNode` (highest weight, ties broken by ascending subject id) → row-normalize per Kamvar → seed personalization vector → iterate to convergence or `maxIterations` → BFS for `seedDistance`. Output deep-frozen per Phase 3.2 frozen-walk.
- `subjectRefToKey` canonicalizes every `SafetySubjectRef` variant; type-prefixed so cross-type collisions are impossible.
- `ReputationGraphConfig` (renamed to avoid collision with Phase 1.64 `transport-admission`'s `ReputationConfig`) carries doctrine-verbatim defaults including the forward-compat fields (`cliquePenaltyExponent`, `pathQualityDamping`) reserved for Phase 1.8.5. `resolveReputationGraphConfig` validates every override against documented ranges; NaN/Infinity throws.
- Determinism: every loop iterates a sorted array of keys; summations happen in sorted order so input array reordering produces byte-identical output; `Math.pow(2, x)` time-decay is IEEE-754 spec-deterministic. **Replay equivalence pinned by byte-identical-thrice test + array-reorder test.**
...
- `ADMISSION_BAND_TABLE` ships doctrine multipliers verbatim (high 2×/2×/0.5×; mid 1×/1×/1×; low 0.5×/0.5×/1.5×; untrusted 0.25×/0.25×/2×). `applyAdmissionBand(baseline, score)` composes the multipliers into the Phase 1.64 engine's per-peer rate-limit-bucket params. **Engine math unchanged** — integration is parameter-only, no algorithm duplication.
- `getCurationDownrankFactor(score)` returns a linear `[CURATION_FLOOR=0.1, 1]` multiplier. **Doctrine non-negotiable #7 ("Reputation never causes silent deletion") enforced via floor** — factor NEVER reaches 0 across full sweep + adversarial inputs (pinned).
- `computeSpamGateDecision(input, config?)` implements the three-condition AND: flags only when `low score AND far from seed AND no positive attestation`. **Fail-open on unknown input** (NaN / non-numeric / non-boolean) returns `'unknown-input'` with `flagSpam: false` — a missing data point CANNOT cause a false-positive spam label. `Number.POSITIVE_INFINITY` seedDistance satisfies the far-from-seed condition (unreachable subjects ARE candidates). 5 stable reason codes for privacy-safe audit per Phase 3.1. Decisions deep-frozen.
- 37 new adversarial tests across band boundaries, monotonicity, frozen tables, three-condition AND with each guard short-circuiting independently, user-override discipline.
