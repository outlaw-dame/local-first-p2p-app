...
| Phase 1.62 - Local user controls | Foundation complete (events + projection + selector shipped; Dexie persistence and PWA wiring deferred) | `@lfp2p/trust-safety` local-controls slice: 7 event kinds (`safety.account.blocked`, `safety.account.muted`, `safety.domain.blocked`, `safety.keyword.muted`, `safety.thread.muted`, `safety.post.hidden`, `safety.label.preference.set`) with `apply`/`revert` action discriminator, pinned `lfp2p.local-control-event.v1`. `assertLocalControlEnvelopeScope` enforces private-by-default scope (device-local / account-local only). `safety.keyword.muted` allows only `substring` or `word` match — user-supplied regexes are excluded as a ReDoS defense and the selector never compiles a regex. `safety.label.preference.set` is restricted to user-facing actions and rejects infrastructure actions like `reject-transport`. `LocalControlState` frozen snapshot with 7 keyed projection tables plus `appliedEventIds` for idempotency. `applyLocalControlEvent` is pure/deterministic/freezes returned state; double-apply is a no-op; `revert` supported. `seedLocalControlState` replays an event log producing equal state on every call (store-reopen rebuild). `decideVisibility` returns the most restrictive of `show < downrank < warn < blur-media < collapse < hide`. Word-boundary keyword scanner — no regex compilation. 8 valid + 6 invalid fixtures; 59 new tests including replay equivalence, idempotency, ReDoS guard, prototype-pollution guard, every networked scope rejected with `TS_PRIVATE_LEAK`. Exit report at `docs/implementation/phase-1.62-exit-report.md`. | Dexie projection persistence (local-store), PWA settings UI (apps/pwa), bridge-side transport enforcement of `safety.account.blocked` (Phase 1.64). |
