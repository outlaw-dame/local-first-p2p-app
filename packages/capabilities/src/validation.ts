...
export function validateCapabilityGrant(value: unknown): CapabilityGrantV1 {
  ...
  if (notBefore !== undefined && Date.parse(notBefore) >= Date.parse(expiresAt)) {
    throw capabilityError(
      ...
      'CapabilityGrantV1.notBefore must be before expiresAt'
    );
  }
  const delegationDepth = assertSafeNonNegativeInteger(
    record.delegationDepth,
    'CapabilityGrantV1.delegationDepth'
  );
  const revocationRef =
    record.revocationRef === undefined
      ? undefined
      : validateRevocationRef(record.revocationRef, 'CapabilityGrantV1.revocationRef');
...
export function validateCapabilityInvocation(value: unknown): CapabilityInvocationV1 {
  ...
  const version = assertVersion(
    record.version,
    CAPABILITY_INVOCATION_VERSION,
    'CapabilityInvocationV1.version'
  );
  ...
  const device =
    record.device === undefined
      ? undefined
      : validatePartyRef(record.device, 'CapabilityInvocationV1.device');
  const resource = validateResourceRef(record.resource, 'CapabilityInvocationV1.resource');
  const action = assertOneOf(
    record.action,
    CAPABILITY_ACTIONS,
    'CapabilityInvocationV1.action',
    'CAP_INVALID_ACTION'
  );
  const scope = validateScopeRef(record.scope, 'CapabilityInvocationV1.scope');
  const argumentsDigest =
    record.argumentsDigest === undefined
      ? undefined
      : assertDigest(record.argumentsDigest, 'CapabilityInvocationV1.argumentsDigest');
  ...
  if (expiresAt !== undefined && Date.parse(createdAt) >= Date.parse(expiresAt)) {
    throw capabilityError(
      'CAP_INVALID_TIMESTAMP',
      'CapabilityInvocationV1.expiresAt must be after createdAt'
    );
  }
...
export function validateCapabilityRevocation(value: unknown): CapabilityRevocationV1 {
  ...
  const version = assertVersion(
    record.version,
    CAPABILITY_REVOCATION_VERSION,
    'CapabilityRevocationV1.version'
  );
  ...
  const audience =
    record.audience === undefined
      ? undefined
      : validatePartyRef(record.audience, 'CapabilityRevocationV1.audience');
  const reasonCode = assertOneOf(
    record.reasonCode,
    CAPABILITY_REVOCATION_REASONS,
    'CapabilityRevocationV1.reasonCode',
    'CAP_INVALID_ENUM'
  );
...
export function validatePartyRef(value: unknown, label = 'CapabilityPartyRef'): CapabilityPartyRef {
  ...
  const digest =
    record.digest === undefined ? undefined : assertDigest(record.digest, `${label}.digest`);
  const publicKeyRef =
    record.publicKeyRef === undefined
      ? undefined
      : assertBoundedText(
          record.publicKeyRef,
          `${label}.publicKeyRef`,
...
function validateActions(value: unknown, label: string): readonly CapabilityAction[] {
  ...
  const out = value.map((item, index) => {
    if (item === '*')
      throw capabilityError(
        'CAP_INVALID_ACTION',
        `${label}[${index}] wildcard actions are forbidden in v1`
      );
    const action = assertOneOf(
      item,
      CAPABILITY_ACTIONS,
    ...
    if (seen.has(action))
      throw capabilityError('CAP_DUPLICATE_VALUE', `${label} contains duplicate action ${action}`);
    seen.add(action);
    return action;
  });
...
function validateCaveats(value: unknown, label: string): readonly CapabilityCaveat[] {
  if (!Array.isArray(value))
    throw capabilityError('CAP_INVALID_CAVEAT', `${label} must be an array`);
...
function validateProofRefs(value: unknown, label: string): readonly CapabilityProofRef[] {
  if (!Array.isArray(value))
    throw capabilityError('CAP_INVALID_PROOF', `${label} must be an array`);
  const seen = new Set<string>();
  return Object.freeze(
    value.map((item, index) => {
      const proof = validateProofRef(item, `${label}[${index}]`);
      if (seen.has(proof.proofId))
        throw capabilityError(
          'CAP_DUPLICATE_VALUE',
          `${label} contains duplicate proofId ${proof.proofId}`
        );
...
function validateJsonValue(value: unknown, label: string): CapabilityJsonValue {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value))
      throw capabilityError('CAP_INVALID_CAVEAT', `${label} must not be NaN or Infinity`);
    return value;
  }
  if (Array.isArray(value))
    return Object.freeze(value.map((item, index) => validateJsonValue(item, `${label}[${index}]`)));
  const record = assertPlainObject(value, label);
...
function assertAllowedKey(key: string, label: string): void {
  if (FORBIDDEN_KEYS.has(key))
    throw capabilityError('CAP_FORBIDDEN_KEY', `${label} uses a forbidden key`);
}

function assertVersion<const T extends string>(value: unknown, expected: T, label: string): T {
  if (value !== expected)
    throw capabilityError('CAP_UNKNOWN_VERSION', `${label} must be ${expected}`);
  return expected;
}
...
function assertBoundedText(
  ...
  label: string,
  max: number,
  code: 'CAP_INVALID_ID' | 'CAP_INVALID_SCOPE'
): string {
  ...
  if (FORBIDDEN_KEYS.has(trimmed))
    throw capabilityError('CAP_FORBIDDEN_KEY', `${label} uses a forbidden value`);
